...
<%
			...
			List<Suggestion>suggestions = suggestionDao.getSuggestionByPostNo(postNo);
			
				if(loginedEmp == null || loginedEmp.getEmpNo() != notice.getEmployee().getEmpNo()) {
				} else {
%>			
...
<%
			...
			if(loginedEmp == null || loginedEmp.getName().equals(notice.getEmployee().getName())) {
...
<div class="modal" tabindex="-1" id="modal-form-posts">
	...
	%>							
								<option value="<%=board.getBoardNo() %>" <%=board.getBoardNo() == notice.getBoard().getBoardNo() ? "selected" : "" %>> <%=board.getName() %></option>
	<%
		}
	%>								
							</select>
					...
					<div class="row mb-2">
						...
						<div class="col-sm-10">
							<input type="text" class="form-control form-control-sm" value="<%=loginedEmp != null ? notice.getTitle() : "" %>" name="title">
						</div>
					</div>
					<div class="row mb-2">
						<label class="col-sm-2 col-form-label col-form-label-sm">작성자</label>
						<div class="col-sm-10">
							<input type="text" class="form-control form-control-sm" readonly="readonly" value="<%=loginedEmp != null ? notice.getEmployee().getName() : "" %>" name="writer">
						</div>
					</div>
					<div class="row mb-2">
						<div class="col-sm-8 offset-sm-2">
							<div class="form-check form-check-inline">
								<input class="form-check-input" type="radio" name="important" value="N" <%=loginedEmp != null && "N".equals(notice.getImportant()) ? "checked" : "" %> >
								<label class="form-check-label">일반</label>
							</div>
							<div class="form-check form-check-inline">
								<input class="form-check-input" type="radio" name="important" value="Y" <%=loginedEmp != null && "Y".equals(notice.getImportant()) ? "checked" : "" %> >
					...
					<div class="row mb-2">
						...
						<div class="col-sm-10">
							<textarea rows="5" class="form-control" name="content"><%=loginedEmp != null ? notice.getContent() : "" %></textarea>
						</div>
					</div>
			</div>
